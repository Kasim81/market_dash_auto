...
│   ├── macro_library_fred.csv         # FRED series IDs (~115 rows; OECD MEI yields + share-price indices + IMF PCPS commodities added 2026-06-10; +9 regime-AA coverage rows 2026-06-15; −7 2026-06-17: 4 dead IDs dropped PR #217 + 3 frozen M2/M3 mirrors replaced by ECB/BoJ/DB.nomics sources PR #221)
...
- **Auth:** None required
- **Used for:** Open-licensed series not on FRED — currently 25 series: 3 Eurostat economic-sentiment surveys (`EU_ESI`, `EU_IND_CONF`, `EU_SVC_CONF`), 5 ISM series (`ISM_MFG_PMI`, `ISM_MFG_NEWORD`, `ISM_SVC_PMI`, `ISM_MFG_INVENTORIES`, `ISM_MFG_PRICES`), 3 Eurostat real-economy series (`EZ_IND_PROD`, `EZ_RETAIL_VOL`, `EZ_EMPLOYMENT`), plus 4 Stage-B T1 fallback rows (IMF/IFS, Eurostat), plus `EA_HICP_CORE_YOY` (Eurostat) and `JPN_CORE_CPI_YOY` (OECD), plus `CHN_M2` (NBS M2 YoY% — added 2026-06-17, replaced frozen FRED mirror), plus **7 OECD COICOP2018 all-items CPI YoY rows added 2026-07-07 (PR #249, CPI-definition split):** `JPN_CPI_YOY` (feeds `JP_INFL1` headline), `CAN_CPI_YOY`, `CHE_CPI_YOY`, `FRA_CPI_YOY`, `ITA_CPI_YOY`, `NLD_CPI_YOY` (national CPI, `_T` all-items expenditure) + `DEU_CPI_YOY` (OECD exposes only the HICP methodology for DE). These are monthly and win the cadence-first merge over the World Bank annual `CPI_YOY` fallback. Library: `data/macro_library_dbnomics.csv`.
