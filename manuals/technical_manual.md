...
│   ├── series_aggregation.csv         # `(source, series_id) -> mean|end|sum` cross-cadence aggregation convention for C16 seam validation (2026-07-30); 1 row — `FRED,IRLTLT01CAM156N,mean` for `CAN_GOV_10Y`. The original 2 populated rows (World Bank CPI, IMF NGDP_RPCH) were removed the same day the owner decided mean-of-monthly-YoY and YoY-of-annual-average-index are different series, not one construction at two cadences; the CAN_GOV_10Y row was added later the same day as a genuine same-construction case
...
| `series_aggregation.csv` | 1 | fetch_macro_economic.py (`_load_aggregation_map`, `_seam_agreement`) | **NEW 2026-07-30 (§2.C C16.1).** `(source, series_id) -> mean\|end\|sum` — declares how a coarser series relates to higher-frequency data for cross-cadence C16 seam validation (cannot be inferred from CSV metadata: coarsening a monthly series to annual by taking December vs. comparing against an annual *average* made identical CPI series look 2–6pp apart). The original two populated rows (World Bank `FP.CPI.TOTL.ZG`, IMF `NGDP_RPCH`) were removed 2026-07-30 when the owner decided mean-of-monthly-YoY and YoY-of-annual-average-index are different constructions, not one series at two cadences — no splice wanted, so no convention needed. Later the same day, `FRED,IRLTLT01CAM156N,mean` was added for `CAN_GOV_10Y` — a genuine single construction (FRED's monthly yield) observed at two cadences, unblocking a 46-year history extension. |
