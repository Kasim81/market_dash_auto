...
| `_select_winner(cands)` | Pick the winning candidate for one `(Country, Col)` group: candidates that mix measure-kinds are a **definition collision** — no guessing, fall back to legacy freshest-then-primary selection; otherwise **finest cadence wins, then lowest tier, then freshest** — a candidate stale by more than 2× its own cadence relative to the group's freshest is dropped first, and a sole candidate always wins regardless of tier |
...
| `_cadence_degradation_event(cands, win)` | **2026-07-27.** Catches the same-tier blind spot in the above: when the finest-cadence candidate and the serving one sit at the *same* tier, a tier-only check sees nothing. Returns `(finer_cand, "CADENCE DEGRADED — <reason>")` when the winner serves at a coarser cadence than the finest **registered** candidate, naming why the finer one lost (no data / definition collision / staleness gate / equal-tier loss); `None` when the winner already is at the finest registered cadence. Rides the same `[FALLBACK]` channel, so `data_audit.py` Section A surfaces it with no scraper change. **Why it exists:** on 2026-07-27 `CHE_CPI_YOY` swapped from the monthly OECD/DB.nomics series to the World Bank **annual** series — both tier 1 — rewriting 3,678 of 4,153 history rows with annual-average values and moving the last observation *backwards* (2026-05-31 → 2025-12-31), and shipped to `main` with no `[FALLBACK]` line. This is the `JP_INFL1` bug class in the one configuration the tier check could not see. Narrow by construction: only 3 columns across the per-source libraries register sources of differing cadence, plus the World Bank / IMF fan-out columns, and an event fires only when the finer source actually fails or goes stale |
