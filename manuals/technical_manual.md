...
├── fetch_macro_economic.py        # Unified raw-macro coordinator (~1,790 lines; §2.C C2 registry dispatch — _SOURCE_HANDLERS + _make_source_handlers replaced the two 27-branch ladders and ~42 template wrappers; +C16 segment-assembly ("splice") engine, 2026-07-29/30 — see §9.4)
├── compute_macro_market.py        # 112 macro-market composite indicators + monthly-hist writer — thin dispatcher over calculators/ since C7 (1,516 lines)
...
├── library_utils.py               # Shared sort-order dicts, FX maps, sort key, SHEETS_* tab sets, INDICATOR_CONCEPT_ORDER (778 lines; +third hist writer for C11 append-stability, 2026-07-18)
├── data_audit.py                  # Daily integrated audit — fetch outcomes + static checks + staleness + registry drift + §3.11 explorer pre-flight + §2.A A9 family-default plausibility/credibility gates (§2.6 v2; 1,858 lines)
...
│   ├── ism_prnewswire.py              # ISM PMI headline/sub-index fallback parsed from official ISM PR Newswire press releases — spliced when the DB.nomics ISM mirror lags or corrupts (252 lines, 2026-07-02)
...
### 9.4 `fetch_macro_economic.py` (~1,790 lines; §2.C C2, 2026-07-09; +C16 segment-assembly engine, 2026-07-29/30)
...
### 9.5 `sources/` package (40 modules — 2 scaffolding-only, ~10,600 lines total)
...
### 9.6 `compute_macro_market.py` (1,516 lines) + the `calculators/` package (C7, 2026-07-16)
