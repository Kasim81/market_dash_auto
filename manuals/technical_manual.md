...
│   ├── istat.py                       # ISTAT (Italy) SDMX-CSV fetcher with vintage (EDITION) resolution, per-series EDITION cache + wildcard-on-miss resolution (395 lines)
...
│   ├── macro_library_ons.csv          # ONS CDID taxonomy paths (14 rows: GBR CPI/CPIH/Core CPI, real GDP, unemployment, employment, AWE, IoP, IoS, RSI, monthly GDP, claimant count, PPI output, CPI index)
...
│   ├── macro_library_imf_sdmx.csv    # IMF Data Portal SDMX keys (11 rows: CHN CPI pair + 4 PI industrial-production rows + 5 QNEA quarterly real-GDP rows; plausibility bands; §2.A A1 + §3.18)
...
- **Used for:** 11 series across three dataflows. **CPI (§2.A A1):** `CHN_CPI_YOY` ← `IMF.STA,CPI/CHN.CPI._T.YOY_PCH_PA_PT.M` (monthly all-items CPI YoY, 1994+, verified 1.20% @ 2026-05 — feeds `CN_INFL1` headline, wins the cadence-first merge over the WB annual fan-out) and `CHN_CPI_INDEX` ← `IMF.STA,CPI/CHN.CPI._T.IX.M` (2020=100, 1993+, verified 104.36 @ 2026-05 — replaced frozen FRED mirror `CHNCPIALLMINMEI`; note the base-year change from the mirror's 2015=100). **PI — Production Indexes (§3.18, 2026-07-08):** monthly SA industrial-production indexes, 2010=100 OL-linked — `DEU_IND_PROD` (1958-01+, replaces the frozen teiis080 mirror), `FRA_IND_PROD` (1956-01+, T1 behind the INSEE tier-0 row), `NLD_IND_PROD` (1956-01+, new), and quarterly `CHE_IND_PROD` (1990-Q1+ SA — Switzerland publishes no monthly IP). The PI `JPN` member is frozen at 2023-11 and is used only as the sister-archive backfill donor for `JPN_IND_PROD` pre-2018 (`scripts/backfill_ind_prod_hist.py`). **QNEA — quarterly national accounts (§3.18):** real GDP levels `B1GQ.Q.SA.XDC.Q` (chain-linked volume, SA, national-currency *units*, not millions) for `DEU_GDP_INDEX` (1991-Q1+), `JPN_GDP_INDEX` (1994-Q1+), `ITA_GDP_INDEX` / `NLD_GDP_INDEX` (1996-Q1+), `CHE_GDP_INDEX` (1995-Q1+) — all verified fresh @ 2026-Q1 on 2026-07-08. Library: `data/macro_library_imf_sdmx.csv` (rows carry `plausible_min`/`plausible_max` for audit Section E).
...
|---|---|---|---|
| **Reference-start** | FRED, ECB, Bundesbank, e-Stat, INSEE, ONS, BLS, StatCan, IMF SDMX (`imf_sdmx`), ISTAT, BoJ | 1st of reference month | ECB: 1st of the **last** month of the quarter; IMF SDMX: 1st of the **first** month |
