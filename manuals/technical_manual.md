...
│   ├── build_review_workbook.py       # Generates docs/dropdown_review.xlsx — two review tabs (macro indicator sub_group proposals + market-data sub-category proposals); output is a review artefact only, does not edit source CSVs
│   ├── ifo_probe.py                   # Reusable diagnostic for ifo.de URL changes — enumerates live download links from the landing pages (see §5 ifo source, §11 Pattern 10)
...
### 9.5 `sources/` package (30 modules — 2 scaffolding-only, ~8,560 lines total)
...
| **D — History preservation** | Catch regressions in the `*_hist.csv` + sister-archive pipeline | **NEW 2026-04-30 (Stage A).** Per-`*_hist.csv` row counts for live + sister + union, plus date ranges. ALERTs on (a) ICE-BofA-bearing file with no sister, (b) sister rows being a strict subset of live (writer regression). Surfaced in `audit_comment.md` so anomalies appear in the daily GitHub Issue notification. |
| **E — Value plausibility** | Catch implausible values that pass staleness checks but are out of credible range | **NEW 2026-06-17 (§2.6 v2 / GDPNow fix PR).** Spot-checks a curated set of series against hard bounds (e.g. GDPNow annualised rate outside ±30%, unemployment outside 0–50%). Flags outliers that could indicate unit-conversion errors, ticker mis-wiring, or publisher anomalies. |
...
| `section_e_plausibility()` | **NEW 2026-06-17 (§2.6 v2 / GDPNow fix PR).** Flags columns whose latest committed value falls outside a curated plausibility band (e.g. GDPNow annualised rate outside ±30%, unemployment outside 0–50%). Returns `{"implausible": [{col_id, series_id, source, value, date, bound, reason}]}`. Catches unit-conversion errors, ticker mis-wiring, and publisher anomalies that pass staleness checks. |
