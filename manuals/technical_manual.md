...
│   ├── macro_library_dbnomics.csv     # DB.nomics series paths (~18 rows)
...
- **Auth:** None required
- **Used for:** Open-licensed series not on FRED — currently 18 series: 3 Eurostat economic-sentiment surveys (`EU_ESI`, `EU_IND_CONF`, `EU_SVC_CONF`), 5 ISM series (`ISM_MFG_PMI`, `ISM_MFG_NEWORD`, `ISM_SVC_PMI`, `ISM_MFG_INVENTORIES`, `ISM_MFG_PRICES`), 3 Eurostat real-economy series (`EZ_IND_PROD`, `EZ_RETAIL_VOL`, `EZ_EMPLOYMENT`), plus 4 Stage-B T1 fallback rows (IMF/IFS, Eurostat), plus `EA_HICP_CORE_YOY` (Eurostat) and `JPN_CORE_CPI_YOY` (OECD), plus `CHN_M2` (NBS M2 YoY% — added 2026-06-17, replaced frozen FRED mirror). Library: `data/macro_library_dbnomics.csv`.
...
| `macro_library_estat.csv` | 5 | sources/estat.py | **NEW 2026-04-30 (Stage D); +5 2026-06-10; revised 2026-06-11; IDs corrected 2026-06-17 to match the live library.** e-Stat statsDataIds (source of truth: `data/macro_library_estat.csv`). Current live IDs: `0004052177?cdCat01=0001000&lang=J` (JPN_IND_PROD — METI IIP; the earlier `0003446463` was the **Cabinet Office Composite Index of Business Conditions**, not the IIP, and was repointed by PR #208), `0003355222?cdCat01=160&cdCat02=100` (JPN_MACH_ORDERS — monthly machinery orders; the earlier `0003355224` was an annual fiscal-year table, repointed by PR #208), `0003138782` (JPN_RETAIL_SALES — METI Current Survey of Commerce; **confirmed 2026-06-17 to be the 2013 annual archive** `CYCLE=年次`, a wrong-table/units defect tracked in the label-vs-data audit), `0002070001` (JPN_HH_EXP — MIC Family Income & Expenditure; WRONG-SLICE, no `cdCat` filters), `0003348427` (JPN_EWS_DI — Economy Watchers Survey national current-DI; WRONG-SLICE, no `cdCat` filters). **2026-06-11 drop:** `JPN_TERT_IND` (METI Tertiary Industry Activity) removed — no `getStatsData` table exists; METI publishes only via Excel file download. Re-add when a file-download fetcher or `sources/meti_jp.py` module exists. |
