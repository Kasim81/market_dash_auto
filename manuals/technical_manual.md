...
│   ├── series_aggregation.csv         # `(source, series_id) -> mean|end|sum` cross-cadence aggregation convention for C16 seam validation (2026-07-30); the 2 original provisional rows (World Bank CPI, IMF NGDP_RPCH) were removed the same day the owner decided mean-of-monthly-YoY and YoY-of-annual-average-index are different series, not one construction at two cadences — then 1 row, `FRED,IRLTLT01CAM156N,mean`, added the same day for the genuinely-one-construction `CAN_GOV_10Y` seam (§2.C C16.1)
...
| `series_aggregation.csv` | 1 | fetch_macro_economic.py (`_load_aggregation_map`, `_seam_agreement`) | **NEW 2026-07-30 (§2.C C16.1).** `(source, series_id) -> mean\|end\|sum` — declares how a coarser series relates to higher-frequency data for cross-cadence C16 seam validation (cannot be inferred from CSV metadata: coarsening a monthly series to annual by taking December vs. comparing against an annual *average* made identical CPI series look 2–6pp apart). Emptied 2026-07-30 when the two provisional rows (World Bank `FP.CPI.TOTL.ZG`, IMF `NGDP_RPCH`) were removed as different constructions, not one series at two cadences — then repopulated the same day with `FRED,IRLTLT01CAM156N` = `mean` once the `CAN_GOV_10Y` seam (BoC daily vs FRED monthly) was confirmed genuinely one construction at two cadences (max\|diff\| 0.0350 vs 0.4332 for end-of-period). |
...
| `section_b_static_checks()` | Run the 7 sub-checks (orphan countries / duplicate ids / missing calculators / missing explorer indicators / missing `_get_col` columns / unadjusted splits / registry drift) |
