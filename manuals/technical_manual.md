...
│   ├── series_aggregation.csv         # `(source, series_id) -> mean|end|sum` cross-cadence aggregation convention for C16 seam validation (2026-07-30); currently empty — the 2 populated rows (World Bank CPI, IMF NGDP_RPCH) were removed the same day the owner decided mean-of-monthly-YoY and YoY-of-annual-average-index are different series, not one construction at two cadences; mechanism kept for a future genuine case
