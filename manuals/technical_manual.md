...
| `macro_market` | 107 composite indicators | Daily snapshot | Derived from above datasets |
| `macro_market_hist` | 107 composite indicators | Weekly history from 2000 | Derived from above datasets |
...
├── data_audit.py                  # Daily integrated audit — fetch outcomes + static checks + staleness + registry drift + §3.11 explorer pre-flight (§2.6 v2; 1,500 lines)
...
├── library_sync.py                # Operator-gated hist↔library prune utility — archives orphan columns then drops them; covers 3 pairs (comp / macro_economic / macro_market) (338 lines). `MACRO_LIBS` covers all 29 single-source registries, now derived from `sources.SOURCE_REGISTRY` (§2.C C2, 2026-07-09; was 25 pre-C2, 21 pre-2026-06-10).
...
│   ├── eurostat.py                    # Eurostat dissemination API direct fetcher (ec.europa.eu) — keyless JSON-stat; DG ECFIN sentiment trio EU_ESI/EU_IND_CONF/EU_SVC_CONF at geo EA21 (§2.A A12, 2026-07-09)
...
│   ├── macro_library_dbnomics.csv     # DB.nomics series paths (18 rows; −3 sentiment trio → sources/eurostat.py 2026-07-09, §2.A A12)
...
│   ├── macro_indicator_library.csv    # 107 macro-market indicator definitions
...
└── .github/workflows/
    ...
    └── ci.yml                     # Offline unit-test gate on pull_request + push-to-main — 12 deterministic modules, 143 tests, no network/keys (forward_plan §2.C C5, 2026-07-08; test_bounded_fill + test_source_registry added 2026-07-08/09; test_library_sync + test_data_audit_critical + test_fetch_backoff added 2026-07-09)
```
...
└─ [try] run_phase_e()                          ← compute_macro_market  → macro_market + macro_market_hist
                                                   ...
                                                   computes 107 composite indicators (z-score, regime, fwd_regime).
```
...
| `compute_all_indicators(cp, mu, mi, supp, dbn)` | Orchestrate all 107 indicator calculations under one try/except per indicator |
...
| `build_hist_df(results)` | One row per date × ~428 columns (107 indicators × 4 values: raw, zscore, regime, fwd_regime). `pd.concat(...).copy()` defragments the wide frame to silence pandas `PerformanceWarning`s in the downstream `reset_index()`. |
...
| `INDICATOR_META` | dict | Maps 107 IDs to `(group, sub_group, category, formula_note, concept, subcategory)` — 6-tuple loaded from CSV. `concept` and `subcategory` were added 2026-04-28 (§2.4) and propagate into the `macro_market.csv` snapshot output. |
...
| `REGIME_RULES` | dict | Maps 107 IDs to regime classification lambdas |
...
| **E — Value plausibility** (§2.6) | Catch a parser / unit regression that lands a physically implausible value the staleness check can't see (e.g. an index level or GDP subcomponent surfacing in a growth-rate slot) | For each column, compare the latest committed value against a plausibility band — built-in defaults overlaid with optional per-row `plausible_min` / `plausible_max` declarations from any `macro_library_*.csv` (library wins). Out-of-band values report as `implausible` and count as a real alert. Added 2026-07-02; shares the `plausible_*` band columns with the `sources/dbnomics.py` §9.5.7 plausibility guard. |
...
| `load_plausibility_bands()` / `section_e_plausibility()` | **NEW 2026-07-02 (§2.6).** `load_plausibility_bands()` merges built-in per-column bands with optional `plausible_min` / `plausible_max` columns declared on any `macro_library_*.csv` (library wins). `section_e_plausibility()` flags every column whose latest committed value falls outside its band as `implausible` — a real alert, since a breach means a parser / unit regression rather than staleness. Same band columns feed the `sources/dbnomics.py` fetch-time plausibility guard (§9.5.7). |
