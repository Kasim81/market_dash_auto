...
| `BRIGHTDATA_API_KEY` | Optional | `sources/ifo.py` (2026-06-12) + `sources/brightdata.py` (2026-07-02). Bright Data Web Unlocker API key. `sources/ifo.py` uses it for the 3rd-strategy anti-bot fallback for ifo workbook downloads (per-run budget capped at 30 calls). `sources/brightdata.py` is a new shared client (2026-07-02) used by `sources/ism_prnewswire.py` for ISM press-release fetching via Bright Data Web Unlocker. Free tier = 5,000 requests/month. If unset, both modules skip the Bright Data path silently. |
| `BRIGHTDATA_ZONE` | Optional | `sources/ifo.py` (2026-06-12) + `sources/brightdata.py` (2026-07-02). Bright Data zone name; defaults to `"web_unlocker1"` when unset. |
