...
| **D — History preservation** | Catch silent regressions where the ICE-BofA workaround writes a shorter history than its sister | **NEW 2026-04-30 (Stage A).** Per-`*_hist.csv` row counts for live + sister + union, plus date ranges. ALERTs on (a) ICE-BofA-bearing file with no sister, (b) sister rows being a strict subset of live (writer regression). Surfaced in `audit_comment.md`. |
| **E — Value plausibility** | Catch implausible committed values (data-corruption or bad unit conversion) | **NEW 2026-06-17.** For each column in the unified hist whose latest committed value falls outside its plausibility band, emit a PLAUSIBILITY_BREACH. Bands come from `PLAUSIBILITY_BANDS_BY_COL` built-in dict (seeded with `US_GDPNOW = [-15, 20]` Q/Q SAAR %) overlaid with optional per-row `plausible_min` / `plausible_max` columns loaded from the library CSV via `load_plausibility_bands()`. |
...
| `section_e_plausibility()` | **NEW 2026-06-17.** Check every column in unified hist whose latest committed value falls outside its plausibility band; uses `PLAUSIBILITY_BANDS_BY_COL` built-in dict (seeded with `US_GDPNOW = [-15, 20]` Q/Q SAAR %) overlaid with optional per-row `plausible_min` / `plausible_max` library CSV columns loaded via `load_plausibility_bands()`. |
