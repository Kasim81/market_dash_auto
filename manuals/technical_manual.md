...
│   ├── series_aggregation.csv         # `(source, series_id) -> mean|end|sum` cross-cadence aggregation convention for C16 seam validation (2026-07-30). The original 2 rows (World Bank CPI, IMF NGDP_RPCH) were removed the same day the owner decided mean-of-monthly-YoY and YoY-of-annual-average-index are different series, not one construction at two cadences; later that day 1 row was added for `FRED,IRLTLT01CAM156N,mean` (`CAN_GOV_10Y`'s monthly FRED leg is a genuine same-construction mean of the daily BoC series, recovering 46 years of history)
...
| `series_aggregation.csv` | 1 | fetch_macro_economic.py (`_load_aggregation_map`, `_seam_agreement`) | **NEW 2026-07-30 (§2.C C16.1).** `(source, series_id) -> mean\|end\|sum` — declares how a coarser series relates to higher-frequency data for cross-cadence C16 seam validation (cannot be inferred from CSV metadata: coarsening a monthly series to annual by taking December vs. comparing against an annual *average* made identical CPI series look 2–6pp apart). The original two rows (World Bank `FP.CPI.TOTL.ZG`, IMF `NGDP_RPCH`) were removed 2026-07-30 when the owner decided mean-of-monthly-YoY and YoY-of-annual-average-index are different constructions, not one series at two cadences — no splice wanted, so no convention needed. Later the same day, `FRED,IRLTLT01CAM156N,mean` was added for `CAN_GOV_10Y`: its monthly FRED leg is a genuine same-construction mean of the daily BoC series (max\|diff\| 0.0350 vs 0.4332 for end-of-period), recovering 46 years of history. |
