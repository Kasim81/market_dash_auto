...
**§2.C sequencing (unified with §2.A):** operational first — **A10 + A11 ✅ done 2026-07-08** (PR #255, plus the C4/C5 structural openers) and **A1 ✅ done 2026-07-08** (CPI via the new IMF SDMX module; PPI accepted gap) — next the **A12** Eurostat-mirror follow-up and the **A13** e-Stat CPI slice-pinning (both need capabilities this sandbox lacks: `ec.europa.eu` egress / `ESTAT_APP_ID`) — then the remaining structural track **C3 → C2 → C1** → (C9/C10 anytime) → §5 multifreq. C1 lands last of the core structural items by dependency convenience (C2 makes it easier), but it is the highest-value structural item — pull it forward if another freeze appears before the registry work is done. Multifreq de-risking: C2 and C1 are effective prerequisites (one registry beats two 27-branch ladders for a migration touching every source path; precedence enforcement defines the correctness contract a multi-frequency merge must preserve), and C5's test gate is what lets a migration that size proceed in reviewable steps.
