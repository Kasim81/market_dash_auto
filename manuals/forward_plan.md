...
### Repo file-hygiene — doc-deletion sweep (2026-07-14, PR #269)

**Done this pass.** Retired five superseded audit/handover files, each with its inbound references rewired to a surviving home (grep-verified zero dangling refs; `py_compile` + `test_tier_merge.py` green; no `data/*.csv` deleted — note-field edits only):
