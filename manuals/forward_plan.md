...
**A9. Build a comprehensive data-credibility check test that runs regularly.** 🟢 **Core shipped 2026-07-15; cross-series assertions are the remaining increment.** *(Raised 2026-06-17 from the Atlanta Fed GDPNow workflow-failure investigation — a ~24% `US_GDPNOW1` nowcast that no audit section and no test caught.)* Delivered this pass:
  ...
  - **Remaining increment (deferred):** the cross-series structural assertions a single band can't express — a YoY that should track its own MoM cumulation; a spread that should equal leg A − leg B (e.g. `US_ISM2`); an index level whose implied period-over-period change is within bounds (the "jump check" for the unbanded level families). These are more bespoke / higher false-positive risk, so they're a clean second increment on top of this foundation. Effort remaining: M.
