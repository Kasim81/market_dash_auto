...
- **B7. UK Building Permits (dwelling starts)** — ✅ **Done 2026-07-16, re-sourced to ONS.** MHCLG's live tables (Table 213) sit behind `www.gov.uk`/`assets.publishing.service.gov.uk`, which are **egress-policy-blocked for this pipeline** (403 CONNECT from sandbox, agent proxy, and Jina; Bright Data returns empty; the Content API's rotating-hash attachment can't be resolved without reaching gov.uk). FRED's UK permits series (OECD MEI: `ODCNPI03GBQ659S`, `GBRPERMITQISMEI`, `WSCNDW01GBQ470S`) are all **discontinued** (~2020-2023). Instead wired the free UK-wide successor on the reachable `ons.gov.uk`: the ONS "Indicators of house building, UK" workbook (dataset `ukhousebuildingpermanentdwellingsstartedandcompleted/current`, stable filename `indicatorsofukhousebuilding.xlsx`, quarterly, actively maintained — v20 = 2026-04-22). New `sources/ons_housing.py` reuses the B8 two-step beta-`/data`→`/file` fetch but a distinct dates-DOWN parse (Period column = quarter ranges like `Jan - Mar 1978`, a revision-flag column precedes it, `[x2]`/`[c]` suppression markers skipped). Series-id `<dataset_uri>|<sheet>|<value_header>`. Wired: `macro_library_ons_housing.csv` (col `GBR_DWELLINGS_STARTED`, sheet `1a` = UK, value `Started - All Dwellings`, tier 0), `SourceSpec("ONS Housing", …)`, coordinator import+handler, explorer allowlist, `test_ons_housing_smoke.py`. Verified live 2026-07-16: 184 quarterly obs 1978-Q1 → 2025-Q4 = **38,010** dwellings started. (NSA quarterly level; `_is_level_price_units` is False for it so the A9 jump check correctly does not fire.)
...
**C7. Split `compute_macro_market.py` into a `calculators/` package — ✅ done 2026-07-16.** Moved all 112 `_calc_*` functions out of the 2,679-line monolith into per-region modules under `calculators/` — `us.py` (37), `europe.py` (19, EU/DE/IT/FR), `uk.py` (9), `japan.py` (9), `asia.py` (12, AS/CN), `fx.py` (10), `monetary.py` (5, M-series), `global_.py` (11, GL/GLOBAL/CA/AU) — plus `calculators/common.py` for the shared helpers (`_p`, `_log_ratio`, `_to_weekly_friday`, `_rolling_zscore`, `_get_col`, `_arith_diff`, `_sum_log_ratio`, `_yoy`, `_taylor_gap`) and the three constants they need (`ZSCORE_WINDOW`, `ZSCORE_MIN_PERIODS`, `_ALIGN_FFILL_LIMIT`). `compute_macro_market.py` (now 1,495 lines) keeps the `_ALL_CALCULATORS` registry (thematic sub-dicts unchanged — they resolve the calcs via `from calculators.<region> import *`), `make_result`/regime assignment, the output builders, and `main`. Pure code move, **no behaviour change** — proven by `test_calculators_parity` (§C7 harness): a golden fingerprint (hash / EMPTY / RAISED:<type>) of all 112 calc outputs over committed data + fixed synthetic supp, frozen *before* the move and asserted identical after (108/112 hashed). The three intra-region cross-calc calls (EU/JP/UK Taylor→Nowcast) stay within their module, so there are no cross-module calc deps. Wired into the ci.yml offline gate.
...
**Root cause pinned 2026-07-16 (rigorous).** The dominant driver is **non-deterministic physical column order**, *not* float noise. Proof: two consecutive daily commits with an **identical 326-column set** (`37e352ec` → `98905bfa`) still differ in **3,735 / 4,167 lines** (~90%), with the header first diverging at column 114 (`FRA_CLI` vs `AUS_CLI`). Line-based git diff sees the whole file change whenever columns shift, even with no schema change. Origin: `fetch_macro_economic.py` builds `col_order` by *first-appearance* while iterating indicators + their **fan-out** series (`_history_for_indicator` → `series_dict.items()`), and the fan-out aggregators (OECD CLI, World Bank, IMF) emit their per-country columns in a non-deterministic order each run; nothing pins a canonical order before write. This is purely a git artifact — consumers read by column *name*, so nothing is functionally wrong. (The display order in `index_library.csv` `sort_key` / `INDICATOR_*_ORDER` is a *separate*, presentation-only ordering used by the explorer and is unaffected.) Secondary driver: `_write_hist_payload` writes floats at full repr precision (no `float_format`), so ~27 columns also wobble in the last ULP.

**The fix (its own PR; core-writer, verify against real data).** Two parts, both at the writer/assembly layer:
  1. **Deterministic column order.** Pin the physical `*_hist.csv` column order to a canonical order — preferably the library `sort_key` order the user already defines (so the stored file matches the intended order *and* is stable), with fan-out columns given a deterministic sub-order (e.g. sorted by country code). Because the order is fixed **before** `columns = list(df.columns)` builds the per-column metadata prefix rows, metadata stays aligned to data (the reason this must live at assembly time, not inside `_write_hist_payload` — reordering after the meta rows are built would misalign them). Apply to all three hist writers: `macro_economic_hist`, `macro_market_hist`, `market_data_comp_hist`, plus the `_x` sister archives (same `write_hist_with_archive` choke point).
  2. **Canonical `float_format="%.8g"`** in `_write_hist_payload` (covers live + sister). 8 sig figs preserves all meaningful precision and kills last-ULP wobble.

  One-time cost: the first post-merge run reformats existing floats + reorders columns once (a single large diff), after which daily diffs collapse to only genuinely-changed cells (recent dates + real source revisions). Verification offline: idempotence (write committed data twice → byte-identical), column-order preservation, value fidelity (round-trip within precision), plus the existing `test_library_utils_hist` / `test_bounded_fill` gates; final byte-stability confirms on the first daily CI run. Effort: M.

Then optionally: (2) move only the append-only `_hist_x` sister archives to compressed Parquet (`pyarrow`, one dependency; `_hist` heads stay CSV); (3) alternative if zero format change wanted: a machine-commit `data` branch squashed monthly. Resolve the §3.6a `keep="first"` self-heal question before (2) changes the archive format. **The bigger question — should the hist store move off CSV to a database entirely — is captured as its own decision in §3.20.**
...
**Status:** ✅ **Wired 2026-07-16.** New `sources/equity_pe.py` orchestrator resolves trailing/forward P/E for the major-index ETF set (SPY/QQQ/DIA/IWM/MDY/EFA/EEM/VGK/EWJ) through a **FactIQ → yfinance → Alpha Vantage** source chain, and `fetch_pe.py` upserts the result into `data/equity_pe_snapshot.csv` under each UTC date — a growing daily P/E history (yfinance exposes only a snapshot, so accumulating it daily is how the history is built for free). FactIQ was evaluated and dropped: its warehouse is macro/labour/trade/SEC with no equity-index P/E series, and as a session-scoped MCP tool it is not reachable from headless CI in any case — so the runtime chain is yfinance `.info` (`trailingPE`/`forwardPE`, primary, no quota) → Alpha Vantage `OVERVIEW` (fallback; `sources/alpha_vantage.py` scaffold). Storage is one row per (`asof_date`, `ticker`) with same-day re-runs upserting, not duplicating. Wired into `update_data.yml` (new continue-on-error P/E step + `equity_pe_snapshot.csv` in the commit allowlist + smoke test) and `ci.yml` (`test_equity_pe` offline gate — source-chain preference, drop-on-miss, and upsert/no-wipe logic, all dependency-injected). yfinance's crumb-gated `.info` path is sandbox-blocked, so live values validate on the first CI run (`test_equity_pe_smoke` skips when no source resolves). Historical CAPE/Shiller PE remains a separate track (§3.13). Prior scaffold shipped 2026-06-10: Alpha Vantage scaffold + smoke test landed (`sources/alpha_vantage.py` with `fetch_overview()` + `get_pe_ratios()`, `data/macro_library_alpha_vantage.csv` empty schema, `test_alpha_vantage_smoke.py` exercising `OVERVIEW` for `SPY`; smoke step in `update_data.yml` extended to expose `ALPHAVANTAGE_API_KEY` and run the new test). Free-tier quota is 25 requests/day so a per-ticker daily pull is feasible for the ~5-row major-index ETF set (SPY/QQQ/IWM/EFA/EEM) once we settle on the storage shape — likely a new `data/equity_pe_snapshot.csv` rather than mixing snapshot-only PE/forward-PE values into the time-series `macro_economic_hist`. Population + writer remain outstanding pending that schema decision. Historical PE / CAPE (multpl.com / shillerdata) is a separate track — see §3.13.
...
**Progress 2026-07-16:** **Completed the iShares STOXX Europe 600 sector ETF set.** The library already carried 9 of these sectors (Banks EXV1, Telecom EXV2, Technology EXV3, Health Care EXV4, Energy EXH1, Consumer Staples EXH3, Industrials EXH4, Utilities EXH9, Real Estate EXI5 — a partial set from the §3.1.8 Stage F review, which had *deferred* completing it over a ticker-mapping mix-up documented in `community_datasets_review.md` Note 1). Added the **9 genuinely-missing sectors** — Automobiles & Parts (EXV5), Basic Resources (EXV6), Chemicals (EXV7), Construction & Materials (EXV8), Travel & Leisure (EXV9), Insurance (EXH5), Media (EXH6), Personal & Household Goods (EXH7), Retail (EXH8) — bringing the set to a complete **18 sectors**, the European counterpart to the US `XL*` sectors. Each new ticker verified live via the Yahoo chart API (`query1.finance.yahoo.com/v8/finance/chart/<t>` with a browser UA — yfinance's own `curl_cffi` path is sandbox-blocked, but that endpoint is reachable): name, `EUR` currency, 2008-01-02 inception. New rows match the **existing STOXX-sector convention** exactly (`units=Index`, `asset_subclass=Equity Sector`, `region=Europe`, `country_market=Europe`, `market_cap_focus=Broad`, `proxy_flag=False`, `data_source=yfinance TR`, `CONFIRMED`, GICS `sector_style` so EU sectors align with the US `XL*` rows for cross-region comparison). EUR already in `COMP_FX_TICKERS`, so no `library_utils.py` change. *(Correction: the initial commit `ba9ace3` mistakenly re-added all 17 sectors in a divergent format — it missed the 9 pre-existing rows — creating 8 duplicate tickers; the follow-up removed those and kept only the 9 new sectors in the house convention.)* Remaining categories below (EM regional, UK style, additional Asia) still open for a future incremental pass.
...
### 3.20 Data Storage Strategy — CSV vs Database (open decision)

**Priority:** Medium — architectural. Raised 2026-07-16 off the back of the C11 churn diagnosis. **Not yet decided.** C11 (§2.C) is the *cheap in-place* answer; this section frames the *bigger* question — should the hist store move off flat CSV to a database — and evaluates every free option that fits a **Claude-authored + GitHub-Actions-run** workflow.

**The constraint that shapes everything.** The pipeline's core design is *"the git repo is the database"*: every hist CSV is versioned in git (full history, byte-reproducible, diff-reviewable), served statically via GitHub Pages, consumed by regime-AA and `build_html.py` **by reading the committed files**, with **no external service, secret, or network dependency** at read time. Any storage change is really a choice about how much of that model to keep. Two further hard constraints: (a) the daily writer is a **single-writer batch** (GitHub Actions cron), so multi-writer concurrency is a non-issue; (b) **outbound network egress from the Claude sandbox is policy-gated** (the same allowlist that blocks gov.uk) — so a store I cannot reach from the sandbox is one I cannot *develop or audit* directly, only wire blindly for CI. That egress limit is the single biggest discriminator below.

**Options (all free) and how each scores on "Claude + GitHub can work on it together":**

| Option | In git repo? | Diff-reviewable | Repo bloat | Reachable from Claude sandbox | GH Actions | Migration cost |
|---|---|---|---|---|---|---|
| **CSV + C11** (status quo, fixed) | ✅ yes | ✅ text lines | Low once C11 lands | ✅ local files | ✅ trivial | ~none (C11 only) |
| **Parquet + DuckDB** (in-repo files) | ✅ yes | ❌ binary | **Lowest** (columnar, ~5–10× compression; year-partitioned → only changed partitions re-blob) | ✅ `pip duckdb/pyarrow`, no egress | ✅ | Medium (rewrite writers + readers) |
| **SQLite** (single `.db` in repo) | ✅ yes | ❌ binary | Medium (whole `.db` re-blobs daily) | ✅ stdlib `sqlite3`, no egress | ✅ | Medium |
| **DuckDB** (single `.duckdb` in repo) | ✅ yes | ❌ binary | Medium | ✅ `pip duckdb`, no egress | ✅ | Medium |
| **Hosted Postgres — Neon / Supabase** (free tier) | ❌ external | ❌ (query, not diff) | **None** (out of git) | ⚠️ **likely blocked** by egress policy | ✅ (secret conn-string) | High |
| **Turso / libSQL** (hosted SQLite, free tier) | ❌ external | ❌ | None | ⚠️ **likely blocked** by egress | ✅ | High |

**Per-option pros / cons:**

- **CSV + C11 (baseline).** *Pros:* keeps every property of the current model (git-versioned, byte-reproducible, human-diffable, Pages-served, zero deps); C11 kills the churn for ~M effort; regime-AA + dashboard unchanged. *Cons:* still a flat wide file (no indexes/SQL; the 4k×340 shape is near the comfortable ceiling for CSV); audit checks stay hand-rolled pandas. **Best when the goal is "stop the churn and change nothing else."**
- **Parquet + DuckDB (in-repo).** *Pros:* biggest bloat win (columnar compression + year-partitioning means a daily update re-blobs only the current-year partition, not the whole history); DuckDB gives full SQL over the Parquet with *zero import step* and no egress, so the audit's internal checks collapse to short queries (the §8.3.1 idea); still fully in-git and Actions-native; I can develop/query it here. *Cons:* binary → loses text diffs (a `git diff` no longer shows "what changed"; you'd verify via a query instead); one dependency (`duckdb`/`pyarrow`); readers (regime-AA, `build_html`) must switch from `read_csv`. **Best when the goal is "shrink the repo and get SQL, keep it in git, accept opaque diffs."**
- **SQLite (in-repo).** *Pros:* zero-dependency (Python stdlib), universal tooling, real SQL + indexes, trivially works in Actions and from my sandbox; a natural fit for the 3-table `series`/`data_points`/`dimensions` shape. *Cons:* row-oriented, so less compression than Parquet and the whole `.db` re-blobs each commit (bloat similar to CSV, minus the reformat churn); binary → no text diff; binary merge conflicts are unresolvable (rare here — single writer). **Best when the goal is "a real embedded DB with SQL and no new dependency," and repo size isn't the top concern.**
- **DuckDB (single-file, in-repo).** *Pros:* same SQL ergonomics as SQLite but columnar/OLAP — far faster for the wide time-series aggregations this pipeline and its audit do; reads/writes Parquet natively. *Cons:* the single `.duckdb` file format is less universally readable than `.sqlite`, still binary/no-diff, and the on-disk format has had breaking version bumps — pin the version. **Best when analytics speed matters and you want one file rather than a Parquet directory.**
- **Hosted Postgres — Neon / Supabase (free tier).** *Pros:* a real relational DB with concurrent access and a *live queryable endpoint* (the dashboard could query current data directly); no repo bloat at all (data leaves git); generous-ish free tiers (Neon ~0.5 GB, scales to zero; Supabase ~500 MB, auto-pauses on inactivity but the daily cron keeps it warm). *Cons:* **breaks the "repo is the database" model** — no git history/reproducibility of the data, so you now need DB backups; adds a connection-string **secret** and an availability dependency on a vendor's free tier; and critically **I likely cannot reach it from the sandbox** (egress-gated), so I could only wire it blind for CI and never develop/audit against it interactively; GitHub Pages can no longer serve the data as static files. **Best only if the project genuinely needs a live multi-consumer queryable API.**
- **Turso / libSQL (hosted SQLite, free tier).** *Pros:* SQLite-compatible so the app code is simpler than Postgres; large free tier; edge-distributed. *Cons:* same external-dependency + **sandbox-egress-blocked** caveats as Neon; smaller ecosystem; still out of git. **Best only if you want a hosted option but prefer SQLite semantics over Postgres.**

**Recommendation (for decision).** Two-step, not either/or:
1. **Now: do C11 on CSV.** It removes the churn while preserving git-versioning, text diffs, Pages serving, and zero-dependency reads — and it's the cheapest path. Nothing else is blocked by it.
2. **Later, if query power / repo size becomes the pain: adopt DuckDB-over-Parquet as an in-repo analytics layer** (keep the CSV heads *or* move wholesale), **not** an external hosted DB. Rationale: Parquet+DuckDB keeps data in git and — decisively — is **reachable from the Claude sandbox with no egress**, so I can build and audit it interactively; a hosted Postgres/Turso would be developable only blind-through-CI given the egress policy, and would trade away the reproducibility that the whole pipeline is built on. Reserve hosted Postgres (Neon) strictly for a future world where a *live external consumer* needs to query the data — a product requirement this project does not have today.

*(Related prior notes: the archived FactIQ teardown's §8.3.1 "3-table DuckDB store" idea and §5 multifreq's native-frequency-storage direction both point the same way — DuckDB/columnar, in-repo, when the flat-CSV shape is finally outgrown.)*
