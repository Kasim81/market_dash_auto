...
- `data/macro_library_sec_edgar.csv` — every SEC EDGAR ticker/metric pair (2026-06-15)
...
| **Phase ME — Macro-Economic** (unified) | Single raw-macro data layer covering FRED / OECD / World Bank / IMF / DB.nomics / ifo / BoE / ECB / BoJ / e-Stat / LBMA / NY Fed / SEC EDGAR. Replaces retired Phase A / B / C / D. | `fetch_macro_economic.py` + `sources/{fred,oecd,worldbank,imf,dbnomics,ifo,boe,ecb,boj,estat,lbma,nasdaq_data_link,ny_fed,sec_edgar,countries}.py` → `macro_economic`, `macro_economic_hist` | Production |
...
- **DB.nomics** — 17 series total: 3 Eurostat survey (EU_ESI / EU_IND_CONF / EU_SVC_CONF), 5 ISM (Manufacturing / New Orders / Services / **ISM_MFG_INVENTORIES** / **ISM_MFG_PRICES**, +2 2026-06-15 PR #206), 3 Eurostat real-economy (industrial production / retail volume / employment), plus 4 Stage B T1 fallback rows (IMF/IFS for `GBR_BANK_RATE` and `EA_HICP`; Eurostat for `DEU_IND_PROD` etc.), plus 2 additional rows. Library: `data/macro_library_dbnomics.csv`.
...
- **ECB Data Portal** (Stage D, 2026-04-30; +3 2026-06-15 PR #204/206) — 6 Euro-area series: Deposit Facility Rate (FM dataset), AAA yield curve 2Y / 30Y (YC dataset), **EZ_CISS** (systemic stress, CISS dataset), **EZ_INFL_EXP_12M** (consumer inflation expectations, CES dataset), plus 1 additional series. Library: `data/macro_library_ecb.csv`. Closes the DE 2Y bund-yield gap and the EA deposit-rate forcing-function row.
...
- **ONS Zebedee /data API** (2026-05-28; +4 2026-06-10; +1 2026-06-11; +2 2026-06-15 PR #205) — 13 UK series: CPI (D7G7), CPIH (L55O), real GDP (ABMI), unemployment (MGSX), employment (LF24), AWE regular pay (KAI9), Core CPI (DKO8 → GBR_CORE_CPI_YOY, blended into UK_INFL1), Index of Production (K222), Index of Services (S2KU), Retail Sales Index (J5EK), monthly real GDP (ECY2 → GBR_GDP_MONTHLY, feeds UK_NOWCAST1), **GBR_CLAIMANT_COUNT** (BCJD), **GBR_PPI_OUTPUT** (GD6Y). Keyless. Library: `data/macro_library_ons.csv`.
...
- **NY Fed Staff Nowcast** (2026-06-12, §3.1.4) — 1 US series: `US_NYFED_NOWCAST` (current-quarter real GDP growth nowcast, weekly, 2016+). Keyless Excel download from the NY Fed medialibrary CDN; rightmost-non-null per publication row tracks the current-vintage headline. Library: `data/macro_library_ny_fed.csv`. Feeds `US_NOWCAST1`.
...
| `macro_library_fred.csv` | ~122 | Phase ME | `sources/fred.py` (incl. 17 OECD MEI rows §3.12 + 14 IMF PCPS commodity rows §3.9.1, 2026-06-10; +9 Bucket A regime-AA gap rows 2026-06-15 PR #201) |
...
| `macro_library_dbnomics.csv` | 17 | Phase ME | `sources/dbnomics.py` (incl. 4 Stage B T1 fallback rows; +ISM_MFG_INVENTORIES + ISM_MFG_PRICES 2026-06-15 PR #206) |
...
| `macro_library_ecb.csv` | 6 | Phase ME (Stage D, 2026-04-30; +3 2026-06-15 PR #204/206) | `sources/ecb.py` (ECB Data Portal — Deposit Rate, AAA yield curve 2Y/30Y, EZ_CISS systemic stress, EZ_INFL_EXP_12M consumer inflation expectations) |
...
| `macro_library_ons.csv` | 13 | Phase ME (2026-05-28; +Core CPI + IoP/IoS/RSI 2026-06-10; +monthly GDP 2026-06-11; +Claimant Count + PPI 2026-06-15 PR #205) | `sources/ons.py` (ONS Zebedee — GBR CPI/CPIH/Core CPI, real GDP, unemployment, employment, AWE, IoP `K222`, IoS `S2KU`, RSI `J5EK`, monthly GDP `ECY2`, GBR_CLAIMANT_COUNT `BCJD`, GBR_PPI_OUTPUT `GD6Y`) |
...
| `macro_library_ny_fed.csv` | 1 | Phase ME (2026-06-12; §3.1.4) | `sources/ny_fed.py` (NY Fed Staff Nowcast — US_NYFED_NOWCAST, current-quarter GDP nowcast, weekly, keyless medialibrary CDN Excel download) |
| `macro_library_sec_edgar.csv` | 68 | Phase ME (2026-06-15) | `sources/sec_edgar.py` (SEC EDGAR XBRL — 34 US-filer pilot pure-plays × revenue + diluted EPS; `equity_fundamentals.csv` isolated output; keyless fair-access UA) |
...
**Read order in `fetch_macro_economic.py`:** `fred → oecd → worldbank → imf → dbnomics → ifo → boe → ecb → boj → estat → nasdaqdl → lbma → boc → statcan → ons → bundesbank → abs → istat → bls → insee → bdf → alpha_vantage → shiller → french → jst → atlanta_fed → ny_fed → sec_edgar`. Each `sources/*.py` exposes `load_library() -> list[dict]` returning the unified indicator schema (`source`, `source_id`, `col`, `name`, `country`, `category`, `subcategory`, `concept`, `cycle_timing`, `units`, `frequency`, `notes`, `sort_key`). Last writer wins per column — this is the implicit fallback mechanism documented in `data/source_fallbacks.csv`.
