...
| `macro_library_jst.csv` | 39 | Phase ME (2026-06-10; §3.13; −1 2026-06-11) | `sources/jst.py` (Macrohistory R6 `.dta` — 10 priority economies × 4 columns: cpi / gdp (nominal) / eq_tr / ltrate, annual; CAN_EQUITY_TR_JST dropped 2026-06-11) |
...
- ✅ **JST nominal GDP relabeling (PR #222):** `data/macro_library_jst.csv` — all 10 `<ISO>_GDP_JST` rows corrected: name "Real GDP" → "Nominal GDP"; units "<ccy> millions (real)" → "<ccy> millions (nominal)". The JST `gdp` column is nominal GDP in local currency millions at current prices — not real/deflated GDP. `sources/jst.py` docstring updated ("UK real GDP" → "UK nominal GDP"). Affects all 10 priority economies (USA, GBR, DEU, FRA, ITA, JPN, NLD, CAN, AUS, CHE).
- ✅ **Market ticker critical fixes (PR #223):** `data/index_library.csv` — 3 wrong-instrument tickers corrected (NDIA.L → INDY currency GBP→USD, CMOD.L → BCOM.L, ^SP500-6020 → ^SP500-6010 status UNAVAILABLE→CONFIRMED); 9 tickers set UNAVAILABLE (8 EMB country-bond proxies serving identical data + SMALLCAP.NS resolving to wrong instrument).
