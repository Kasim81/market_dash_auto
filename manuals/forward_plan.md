...
Surfaced by the 2026-07-12 FactIQ audit (`handover/audit/findings.csv` + `working_list.csv`; the §12 backlog write-up is archived in git PR #266). This is a `index_library.csv` remediation backlog and belongs to the §3.8 machinery above — it uses the **same `data/removed_tickers.csv` ledger + `data/yfinance_failure_streaks.csv` streak tracker** the weekly review already drives. Three classes:
