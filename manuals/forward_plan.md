...
- **DB.nomics** — 23 series total: 3 Eurostat survey (EU_ESI / EU_IND_CONF / EU_SVC_CONF), 5 ISM (Manufacturing PMI / New Orders / Services / Inventories / Prices Paid), 3 Eurostat real-economy (industrial production / retail volume / employment), plus 3 Stage B T1 fallback rows (IMF/IFS for `JPN_POLICY_RATE` / `CHN_POLICY_RATE`; Eurostat for `DEU_IND_PROD`), plus `JPN_CORE_CPI_YOY` (OECD COICOP2018), plus `CHN_M2` (NBS M_A0D01/A0D0102 — China M2 YoY%, added 2026-06-17), plus **7 OECD COICOP2018 all-items CPI YoY rows (2026-07-07, PR #249 CPI-definition split):** `JPN_CPI_YOY` (feeds `JP_INFL1`) + CAN / CHE / FRA / ITA / NLD national CPI + DEU (HICP). The 2 Eurostat HICP rows (`EA_HICP`, `EA_HICP_CORE_YOY`) were rerouted to the ECB Data Portal 2026-07-08 (§2.A A10 — Eurostat discontinued the slice; DB.nomics Eurostat provider mirror frozen since 2026-01-22, see §2.A A12). Library: `data/macro_library_dbnomics.csv`.
...
| `macro_library_fred.csv` | ~114 | Phase ME | `sources/fred.py` (incl. 17 OECD MEI rows §3.12 + 14 IMF PCPS commodity rows §3.9.1, 2026-06-10; +9 regime-AA coverage rows 2026-06-15; −7 2026-06-17: 4 dead IDs dropped PR #217 + 3 frozen M2/M3 mirrors replaced PR #221; −1 2026-07-08: dead JPN_CPI_INDEX mirror dropped per §2.A A11) |
...
| `macro_library_dbnomics.csv` | 23 | Phase ME | `sources/dbnomics.py` (incl. 3 Stage B T1 fallback rows; +ISM_MFG_INVENTORIES + ISM_MFG_PRICES 2026-06-15; +JPN_CORE_CPI_YOY 2026-06-10; +CHN_M2 NBS 2026-06-17; **+7 OECD COICOP2018 all-items CPI YoY 2026-07-07 PR #249** — JPN/CAN/CHE/FRA/ITA/NLD + DEU-HICP; −2 2026-07-08: EA_HICP + EA_HICP_CORE_YOY rerouted to ECB per §2.A A10) |
