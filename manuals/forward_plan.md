...
- `data/macro_library_ny_fed.csv` — every NY Fed Staff Nowcast series identifier (2026-06-11)
...
- **BoJ Time-Series Data Search API** (Stage D, 2026-04-30; +2 2026-06-10; +5 2026-06-11) — 9 Japan series: Policy Rate (FM01'STRDCLUCON), Tankan Large Mfg DI (JP_TANKAN1), JP PPI (JPN_PPI), Services PPI (JPN_SPPI), plus 5 Tankan sub-DIs added 2026-06-11 for the `JP_TANKAN_SPREAD1` / `JP_TANKAN_SVC1` / `JP_TANKAN_FWD1` Phase E indicators (Large Mfg Forecast, Large Non-Mfg, Large Non-Mfg Forecast, Small Mfg, Small Non-Mfg DIs). Library: `data/macro_library_boj.csv`.
- **e-Stat REST API** (Stage D, 2026-04-30; +5 2026-06-10; revised 2026-06-11) — 5 Japan series: METI IIP (0003446463 — verified live), plus 4 PROVISIONAL rows with HIGH-confidence IDs: JPN_MACH_ORDERS (0003355224), JPN_RETAIL_SALES (0003138782), JPN_HH_EXP (0003000807), JPN_EWS_DI (0003348423). `JPN_TERT_IND` dropped 2026-06-11 — no `getStatsData` table exists. Library: `data/macro_library_estat.csv`. Requires `ESTAT_APP_ID` env var (injected via GitHub Actions secret).
...
- **ONS Zebedee /data API** (2026-05-28; +4 2026-06-10; +1 2026-06-11) — 11 UK series: CPI (D7G7), CPIH (L55O), real GDP (ABMI), unemployment (MGSX), employment (LF24), AWE regular pay (KAI9), Core CPI (DKO8 → GBR_CORE_CPI_YOY, blended into UK_INFL1), Index of Production (K222), Index of Services (S2KU), Retail Sales Index (J5EK), and monthly real GDP (ECY2 → GBR_GDP_MONTHLY, 2026-06-11, feeds UK_NOWCAST1). Keyless. Library: `data/macro_library_ons.csv`.
...
- **New York Fed Staff Nowcast** (2026-06-11, §3.1.4) — 1 US series: `US_NYFED_NOWCAST` (real-time Q/Q SAAR GDP growth nowcast, weekly, 2014+). Keyless xlsx download from NY Fed medialibrary static asset. Library: `data/macro_library_ny_fed.csv`. Feeds the `US_NOWCAST1` Phase E indicator as a second-opinion US GDP nowcast alongside `US_GDPNOW1`.
...
| `macro_library_fred.csv` | ~113 | Phase ME | `sources/fred.py` (incl. 17 OECD MEI rows §3.12 + 14 IMF PCPS commodity rows §3.9.1, 2026-06-10) |
...
| `macro_library_boj.csv` | 9 | Phase ME (Stage D, 2026-04-30; +PPI/SPPI 2026-06-10; +5 Tankan sub-DIs 2026-06-11) | `sources/boj.py` (BoJ Time-Series — Policy Rate, Tankan Large Mfg DI, PPI, SPPI, +5 sub-DIs: Large/Small Mfg+NMfg Forecast DIs feeding JP_TANKAN_SPREAD1/SVC1/FWD1) |
...
| `macro_library_ons.csv` | 11 | Phase ME (2026-05-28; +Core CPI + IoP/IoS/RSI 2026-06-10; +monthly GDP 2026-06-11) | `sources/ons.py` (ONS Zebedee — GBR CPI/CPIH/Core CPI, real GDP, unemployment, employment, AWE, IoP `K222`, IoS `S2KU`, RSI `J5EK`, monthly GDP `ECY2` → GBR_GDP_MONTHLY) |
...
| `macro_library_ny_fed.csv` | 1 | Phase ME (2026-06-11; §3.1.4) | `sources/ny_fed.py` (NY Fed Staff Nowcast — US_NYFED_NOWCAST, Q/Q SAAR GDP nowcast, weekly, keyless) |
...
| `macro_indicator_library.csv` | 107 | Phase E | `compute_macro_market.py` (composite indicator registry) — 93 original + `GLOBAL_GOLD1` (§3.9) + 5 per-region inflation composites (`US/UK/EU/JP/CN_INFL1`, §3.1.3) + `US_INFEXP1` + **8 2026-06-11** (`EU_NOWCAST1`, `US_GDPNOW1`, `UK_NOWCAST1`, `US_NOWCAST1`, `JP_NOWCAST1`, `JP_TANKAN_SPREAD1`, `JP_TANKAN_SVC1`, `JP_TANKAN_FWD1`) |
...
- ✅ §3.1.4 `US_NOWCAST1` NY Fed Staff Nowcast — new `sources/ny_fed.py` (348 lines) + `data/macro_library_ny_fed.csv` (1 row: `nyfed_nowcast_us_qoq_saar`). Keyless xlsx download from NY Fed medialibrary static asset. Feeds `US_NOWCAST1` Phase E indicator as second-opinion US GDP nowcast alongside `US_GDPNOW1`. `test_ny_fed_smoke.py` wired into daily CI. `macro_indicator_library.csv` grows from 105 → 106.
- ✅ §3.1.4 `JP_NOWCAST1` Japan nowcast composite — Phase E indicator shipped (equal-weight z of JPN_IND_PROD + JP_TANKAN1 + JPN_RETAIL_SALES + JPN_MACH_ORDERS); degrades gracefully if e-Stat PROVISIONAL rows not yet credentialed. `macro_indicator_library.csv` grows from 106 → 107.
- ✅ ifo Bright Data Web Unlocker fallback — `sources/ifo.py` (344 → 579 lines) now re-routes through Bright Data when ifo.de anti-bot layer blocks the direct download. `BRIGHTDATA_API_KEY` + `BRIGHTDATA_ZONE` wired into `update_data.yml`. Per-run cap = 30 requests against the 5,000 req/month free tier. When unset, falls back silently to blank rows.
- ✅ ISTAT retry budget tightened — `sources/istat.py` timeout 90s → 30s, retries 6 → 3 (commit 77fe006). Prevents ISTAT 503 storms from blocking the full daily run.
...
1. ✅ **Wire Atlanta Fed GDPNow** as `US_GDPNOW1` — `sources/atlanta_fed.py` (366 lines) + `data/macro_library_atlanta_fed.csv` + Phase E indicator shipped 2026-06-11. Direct first-class Phase E indicator on the Growth axis.
2. ✅ **Wire NY Fed Nowcast** as `US_NOWCAST1` — `sources/ny_fed.py` (348 lines, 2026-06-12) + `data/macro_library_ny_fed.csv` (1 row: `nyfed_nowcast_us_qoq_saar`). Keyless xlsx download from NY Fed medialibrary static asset. Smoke test `test_ny_fed_smoke.py` wired into daily CI.
...
5. ✅ **Build JP nowcast composite** — `JP_NOWCAST1` Phase E indicator shipped 2026-06-12. Composition: equal-weight z-scores of `JPN_IND_PROD` + `JP_TANKAN1` (Tankan DI) + `JPN_RETAIL_SALES` + `JPN_MACH_ORDERS`. Degrades gracefully if e-Stat PROVISIONAL rows not yet credentialed.
...
**Acceptance for §3.1.4:** ✅ All 5 nowcast indicators (`US_GDPNOW1`, `US_NOWCAST1`, `UK_NOWCAST1`, `EU_NOWCAST1`, `JP_NOWCAST1`) shipped and appear in `macro_market.csv`. Weekly time series available to the regime classifier's Growth axis (consumer-side work in `../regime_AA_master_plan.docx`).
...
- ✅ **GDP Now (§3.1.4)** — all 5 nowcast indicators shipped: `US_GDPNOW1` (Atlanta Fed, 2026-06-11); `EU_NOWCAST1` (equal-weight z of EZ IP / retail / ESI / industrial confidence, 2026-06-11); `UK_NOWCAST1` (ONS monthly real GDP ECY2 → YoY %, 2026-06-11); `US_NOWCAST1` (NY Fed Staff Nowcast via `sources/ny_fed.py`, 2026-06-12); `JP_NOWCAST1` (equal-weight z of JPN_IND_PROD + JP_TANKAN1 + JPN_RETAIL_SALES + JPN_MACH_ORDERS, 2026-06-12 — degrades gracefully if e-Stat PROVISIONAL rows not yet credentialed).
