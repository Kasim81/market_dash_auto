...
- 🔴 First credentialed `workflow_dispatch` run completed 2026-06-10 (run id `27271070256`). Result: smoke test FAILED — both provisional `MIR1/...` series returned **HTTP 401**. The secret IS reaching the runtime (`sources/bdf.py` would log `"no BDF_API_KEY env var"` if it weren't; we see `[BdF HTTP 401]` instead), so Webstat is rejecting the credential. Three candidate causes, indistinguishable from current logs: (i) the secret value is not a valid Webstat Client ID; (ii) the Webstat app also requires a Client Secret and `BDF_API_SECRET` is unset; (iii) the `X-IBM-Client-Id` header convention doesn't match what this app expects. Diagnostic logging extended same day to surface the 401 body + secret-present flag — next daily run will distinguish the three. The 401 occurs both before and after smoke tests (snapshot + hist + smoke), so the gap is auth, not series-key validity. Until resolved, `FRA_LOAN_RATE_HOUSE` / `FRA_LOAN_RATE_NFC` remain unpopulated.
