...
**Country registry:** `data/macro_library_countries.csv` is the single source of truth for the 14 country codes (USA, GBR, DEU, FRA, ITA, JPN, CHN, AUS, CAN, CHE, EA19, IND, NLD, GLOBAL) and their WB / IMF code mappings. `IND` was added in the 2026-04-26 supplemental refactor for the India 10Y bond yield, with empty `wb_code` / `imf_code` so it doesn't fan out into multi-country queries. `NLD` was added 2026-06-10 (§3.12 priority-10 Netherlands yields/equities). `GLOBAL` carries no WB/IMF mappings and is used for non-country-specific series.
