...
- **DB.nomics** — 25 series total: 3 Eurostat survey (EU_ESI / EU_IND_CONF / EU_SVC_CONF), 5 ISM (Manufacturing PMI / New Orders / Services / Inventories / Prices Paid), 3 Eurostat real-economy (industrial production / retail volume / employment), plus 4 Stage B T1 fallback rows (IMF/IFS for `JPN_POLICY_RATE` / `CHN_POLICY_RATE`; Eurostat for `EA_HICP` / `DEU_IND_PROD`), plus `EA_HICP_CORE_YOY` (Eurostat COICOP2018) and `JPN_CORE_CPI_YOY` (OECD COICOP2018), plus `CHN_M2` (NBS M_A0D01/A0D0102 — China M2 YoY%, added 2026-06-17), plus **7 OECD COICOP2018 all-items CPI YoY rows (2026-07-07, PR #249 CPI-definition split):** `JPN_CPI_YOY` (feeds `JP_INFL1`) + CAN / CHE / FRA / ITA / NLD national CPI + DEU (HICP). Library: `data/macro_library_dbnomics.csv`.
...
| `macro_library_dbnomics.csv` | 25 | Phase ME | `sources/dbnomics.py` (incl. 4 Stage B T1 fallback rows; +ISM_MFG_INVENTORIES + ISM_MFG_PRICES 2026-06-15; +EA_HICP_CORE_YOY + JPN_CORE_CPI_YOY 2026-06-10; +CHN_M2 NBS 2026-06-17; **+7 OECD COICOP2018 all-items CPI YoY 2026-07-07 PR #249** — JPN/CAN/CHE/FRA/ITA/NLD + DEU-HICP) |
...
**Read order in `fetch_macro_economic.py`:** `fred → oecd → worldbank → imf → dbnomics → ifo → boe → ecb → boj → estat → nasdaqdl → lbma → boc → statcan → ons → bundesbank → abs → istat → bls → insee → bdf → alpha_vantage → shiller → french → jst → atlanta_fed → ny_fed`. Each `sources/*.py` exposes `load_library() -> list[dict]` returning the unified indicator schema (`source`, `source_id`, `col`, `name`, `country`, `category`, `subcategory`, `concept`, `cycle_timing`, `units`, `frequency`, `notes`, `sort_key`). Per column, the winning source is now chosen by the tier-aware, cadence-first, staleness-fallback merge (PR #246, 2026-07-03) rather than plain last-writer-wins — this read order only supplies the final tie-break. See `data/source_fallbacks.csv` and tech_manual §9.4.
...
**A1. China inflation cluster (CHN_CPI_INDEX + CHN_PPI) — investigate IMF IFS replacement.** Both currently read FRED OECD-MEI mirrors (`CHNCPIALLMINMEI`, `CHNPIEATI01GYM`) that have frozen — CN headline CPI hasn't moved since 2025-04 (~15 months, age 459d); CN PPI hasn't moved since 2022-12 (~3.5 years, age 1313d). Both feed `CN_INFL1`, which means the China inflation composite is currently running on 1–3-year-old data. **Update 2026-07-07 (P2 discovery during PR #249, see `manuals/2026-07-07-eu-freeze-freshness-china-handover.md`):** three candidate replacements were probed and ruled out — OECD COICOP2018 has no `REF_AREA=CHN` coverage (0 docs); IMF IFS *on DB.nomics* (`M.CN.PCPI_PC_CP_A_PT`) is itself ~1 year stale (last obs 2025-07 when probed 2026-07); NBS on DB.nomics (`NBS/M_A010101…`) publishes CPI only as same-month-last-year=100 indices with recent values NA, and would need a value−100 transform our adapters don't do. `CHN_CPI_YOY` (the split's new column) exists only on the World Bank annual fallback, itself frozen at 2023 — not a fix. Remaining leads, not yet tried: (a) the IMF IFS *direct* SDMX/JSON API (bypassing the DB.nomics mirror, which may lag independently), (b) NBS direct with a small value−100 transform helper. If neither is fresh, formalize `CN_INFL1` as a documented degraded indicator (§1 Known Data Gaps) rather than leaving it silently frozen, and consider a plausibility band so the audit flags further drift. Effort: M (discovery is the work).
...
**A10. `EU_INFL1` silently frozen since 2026-01 — Eurostat HICP source repair (P0).** `_calc_EU_INFL1 = mean(EA_HICP, EA_HICP_CORE_YOY)`; both inputs, plus the 3 Eurostat sentiment series (`EU_ESI`, `EU_IND_CONF`, `EU_SVC_CONF`), stall at the identical `last_obs = 2026-01-02` in the 2026-07-07 audit (confirmed still flat — `EA_HICP` 1.9, `EA_HICP_CORE_YOY` 2.3, unchanged every week 2026-02 through 2026-07-03 in `macro_economic_hist.csv`). This is the same class of bug as the `JP_INFL1` freeze the CPI-definition split fixed (PR #249): a frozen raw input hides behind a composite that still *looks* populated. **Already ruled out:** switching `EA_HICP` to the direct Eurostat dissemination API — probed live and it also returns data only through 2025-12 for this exact `RCH_A/EA20/CP00` slice, so the stall is upstream of DB.nomics (a genuine Eurostat-side dataset/vintage change at year-end 2025, not a mirror bug). **Strongest lead:** the ECB Data Portal (`sources/ecb.py`, already wired for CISS/CES/the AAA yield curve/EZ_M3) publishes euro-area HICP directly (`ICP` dataset, e.g. an `ICP.M.U2.N.000000.4.ANR` annual-rate key + the ex-energy-food core equivalent) — verify it returns fresh (May/Jun 2026) monthly data before repointing `EA_HICP` / `EA_HICP_CORE_YOY` there (add rows to `macro_library_ecb.csv`) or, if a corrected Eurostat slice is found instead, edit the `macro_library_dbnomics.csv` rows in place. Validation gate: the before/after composite tail must show `EU_INFL1` actually moving month-to-month, not just a fresher-looking timestamp. Full discovery notes, ruled-out paths, and the exact DB.nomics/Eurostat series ids probed: `manuals/2026-07-07-eu-freeze-freshness-china-handover.md` (P0 section). Effort: M–L (discovery is the work; the CSV repoint itself is trivial).

**A11. Freshness-honesty pass — widen overrides only where cadence justifies it (P1).** The 2026-07-07 audit run showed 79 issues (78 stale), most of them honest publication lag under too-tight tolerances — exactly the noise that let the genuine A10 freeze hide in the pile. Concrete, not-yet-applied CSV-only changes (full detail + per-row current values in the handover doc's P1 section):
  - **OECD COICOP2018 CPI-YoY family** (7 rows added by PR #249): `JPN_CPI_YOY`, `CAN_CPI_YOY`, `CHE_CPI_YOY`, `FRA_CPI_YOY`, `ITA_CPI_YOY`, `NLD_CPI_YOY`, `DEU_CPI_YOY` in `macro_library_dbnomics.csv` — OECD publishes COICOP CPI ~6–8 weeks in arrears (observed age ~67d vs a 45d default); set `freshness_override_days` to ~90d (cross-check `JPN_CORE_CPI_YOY`, which shows the same cadence).
  - **e-Stat** `JPN_HH_EXP`, `JPN_EWS_DI` in `macro_library_estat.csv`: FIES / Economy-Watchers publish ~5–6 weeks late (observed ~67d vs a 60d override) — set ~75–80d. `JPN_IND_PROD` (123d/60d) is borderline; leave or nudge.
  - **ONS** `GBR_CPI_YOY` in `macro_library_ons.csv`: 95d vs 75d override — set ~90d.
  - **Event-driven policy rates** (§2.A A7 extension): `JPN_POLICY_RATE` (`macro_library_boj.csv`, 18d vs 5d), `CAN_POLICY_RATE` (`macro_library_boc.csv`, 249d vs 7d), `CHN_POLICY_RATE` (`macro_library_dbnomics.csv`, IMF/IFS — also genuinely mirror-stale, so a wide override is honest either way) — set `freshness_override_days` to the ~90–120d decision cadence; also scan `EA_DEPOSIT_RATE` / `GBR_BANK_RATE` for the same pattern.
  - **Dead frozen mirror** `JPN_CPI_INDEX` (`macro_library_fred.csv`, FRED `JPNCPIALLMINMEI`, frozen 2021-06, age 1859d) — renamed from `JPN_CPI` by the CPI split; nothing consumes it now (`JP_INFL1` uses `JPN_CPI_YOY`; no `_calc_*` reads `JPN_CPI_INDEX`). Recommended: drop the row (pure EXPIRED noise, no consumer, no live source). `CHN_CPI_INDEX` is the analogous-looking case but **is** consumed by `CN_INFL1` — handled under A1, not here.
  - **Guardrail:** never widen a tolerance to silence a *live composite* that has gone flat — that's an A1/A10-class source-replacement problem, not a P1 override. Effort: S (CSV edits only); validate with a keyed regen + re-run `data_audit.py` and confirm the STALE/EXPIRED count drops toward the genuinely-dead residue.
