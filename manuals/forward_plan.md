...
| `macro_library_boj.csv` | 5 | Phase ME (Stage D, 2026-04-30; +PPI/SPPI 2026-06-10) | `sources/boj.py` (BoJ Time-Series — Policy Rate, Tankan Large Mfg DI, PR01/PR02 PPI + Services PPI) |
| `macro_library_estat.csv` | 6 | Phase ME (Stage D, 2026-04-30; +5 growth rows 2026-06-10) | `sources/estat.py` (e-Stat — METI IIP + 5 PROVISIONAL: tertiary industry, machinery orders, retail sales, household exp, EWS DI) |
...
| `macro_library_ons.csv` | 10 | Phase ME (2026-05-28; +Core CPI + IoP/IoS/RSI 2026-06-10) | `sources/ons.py` (ONS Zebedee — GBR CPI/CPIH, real GDP, unemployment, employment, AWE, Core CPI ex-energy/food/alc/tob `DKO8`, IoP `K222`, IoS `S2KU`, RSI `J5EK`) |
...
| `macro_library_jst.csv` | 40 | Phase ME (2026-06-10; §3.13) | `sources/jst.py` (Macrohistory R6 `.dta` — 10 priority economies × 4 columns: cpi / gdp / eq_tr / ltrate, annual) |
...
**Read order in `fetch_macro_economic.py`:** `fred → oecd → worldbank → imf → dbnomics → ifo → boe → ecb → boj → estat → nasdaqdl → lbma → boc → statcan → ons → bundesbank → abs → istat → bls → insee → bdf → alpha_vantage → shiller → french → jst`. Each `sources/*.py` exposes `load_library() -> list[dict]` returning the unified indicator schema (`source`, `source_id`, `col`, `name`, `country`, `category`, `subcategory`, `concept`, `cycle_timing`, `units`, `frequency`, `notes`, `sort_key`). Last writer wins per column — this is the implicit fallback mechanism documented in `data/source_fallbacks.csv`.
...
- ⏳ **§3.1 Stage C UK labour series — investigate monthly-cadence CDID variants.** The LMS labour series we wired (`LF24` employment rate, `MGSX` unemployment rate, `KAI9` AWE regular pay) are surfaced via ONS at a quarterly cadence with ~45-day post-quarter-end lag — confirmed empirically by the 2026-06-10 23:43 UTC run (ages 96-159d at run time vs the original 45d tolerance, with no fresher alternative source available: OECD MEI / Eurostat / IMF IFS / ILO are all derivative ONS data with an additional harmonisation step, so they're as-slow or slower). Freshness override bumped to 150d 2026-06-11 to suppress the false-positive STALE alerts. **Open follow-up:** ONS's Labour Force Survey has multiple alternate CDIDs covering substantially the same underlying populations on different release cadences (e.g. `LF31` is a known monthly-cadence variant of the LF24 employment-rate cut). Worth a dedicated session to enumerate the monthly-cadence alternates for each of the three series and decide whether to swap or augment — could shave the labour-market lag from 150d back toward ~75d without changing sources. Not blocking regime-AA work since the current 150d tolerance is honest about the actual cadence; this is a freshness improvement, not a missing-coverage fix.
- ✅ §3.1 Stage C JP growth — 5 PROVISIONAL e-Stat rows lifting the JP Growth column from 3/7 → 8/7 (statsDataIds need live verification once `ESTAT_APP_ID` reaches the runtime).
