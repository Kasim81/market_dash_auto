    ...
    for attempt in range(retries):
        try:
            ...
            if 500 <= resp.status_code < 600:
                wait = 2 ** attempt
                print(f"    [Bundesbank HTTP {resp.status_code}] {series_id} — backing off {wait}s")
                time.sleep(wait)
                continue
            print(f"    [Bundesbank HTTP {resp.status_code}] {series_id} — skipping")
            return None
        except requests.Timeout:
            wait = 2 ** attempt
            print(f"    [Bundesbank timeout] {series_id} — backing off {wait}s")
            time.sleep(wait)
        except requests.RequestException as e:
            print(f"    [Bundesbank request error] {series_id}: {e} — skipping")
            return None

    print(f"    [Bundesbank FAIL] {series_id} — {retries} attempts exhausted")
    return None
